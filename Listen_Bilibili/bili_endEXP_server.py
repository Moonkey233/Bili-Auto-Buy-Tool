...
csrf = "96065e21760fd0b2c9b99c65c9415cd5" #有时效的用户登录验证, 抓包
access_key = "2baffff35a5c7a9272ecc30e8c383391" #有时效的用户登录验证, 抓包
cookies = { #有时效的用户登录验证, 抓包
    "SESSDATA":"45e80afc%2C1678158580%2Ca29e8891",
...
def GenerateSuitListContent(n, add_month = -1):
    """ 生成下单用表单 """
    statistics_ = '{"appId":1,"platform":3,"version":"__version__","abtest":""}'
    statistics = statistics_.replace("__version__", version)
    data_str = urlencode({
...
def suitGet():
    data, length = GenerateSuitBuyContent()
    headers = {
    ...
    try:
        res = requests.get(url = list_url + data, headers = headers , cookies = cookies)
        n = json.loads(res.text)["data"]["page"]["total"]
        return n
    except:
        return 0
    ...
    while True:
            ...
            try:
                ...
                if timer == 0:
                    ...
                    print("\n开始监听..." + "\n装扮名: " + item_name + 
                        ...
                        "\n预期开始抢单号: " + str(expectation) + 
                        "\n剩余间隔: " + str(expectation - true_number) + 
                        "\n时间戳: " + str(time.time()) + 
                        "\n时间: " + time.asctime() +
                        "\nID: " + my_name)
                ...
                if true_number + 1 >= expectation:
                    ...
                    while True:
                        now = time.time()
                        if now - timer > fresh_sec:
                            timer = now
                            try:
                                reponse = requests.get(url = num_url, headers = header)
                                if json.loads(reponse.text)["data"]["surplus"] == 0:
                                    continue
                                ...
                                number = json.loads(reponse2.text)["data"]["rank"][0]["number"]
                                true_number = number if number >= lastest_id else lastest_id
                                if true_number == total:
                                    break
                            except:
                                print("异常")
                            # if true_number + num >= expectation:
                            #     suitPost(num if num <= 6 else 6)
        ...
        if true_number + 1 > expectation:
            ...
            try:
                ...
                winner = "未知, 请前往B站客户端查看尾号拥有者。"

                for i in dic2["data"]["rank"]:
                    if int(i["number"]) == total:
                        winner = i["nickname"]
            ...
            if mail_on:
                ...
                if winner == my_name:
                    isWin = ", 恭喜您抢购成功!"
                send_mail("抢购结束通知", "您监听的B站装扮 " + item_name + " 尾号【" + str(total) + "】抢购结束, 编号拥有者: " + winner + " 成功申请套数: " + str(true_buy_num) + isWin)
            os.system("pause")
            sys.exit(0)